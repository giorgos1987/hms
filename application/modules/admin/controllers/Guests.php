...
class Guests extends Admin_Controller {
	 ...
	 public function ajax_list()
    {
		...
		//echo '<pre>'; print_r($this->guest_model->count_filtered());die;
        $output = array(
                        "draw" => $_POST['draw'],
	...
	function view($id,$tab=false){
		...
		$data['payments']			= 	$this->guest_model->get_payments($id);
		$data['bookings']		=	$this->guest_model->get_bookings($id);
	...
	function form($id = false)
	{
		...
		//gpap validation remove required from email - mobile - password - password confirm
		...
		if($this->input->post('email') == $this->input->post('old_email')) {
		  	$this->form_validation->set_rules('email', 'lanng:email', 'trim|max_length[128]');
		} else {
		  $this->form_validation->set_rules('email', 'lanng:email', 'trim|max_length[128]|is_unique[guests.email]');
		}
		...
		if ($this->input->post('password') != '' || $this->input->post('confirm') != '' || !$id)
		{
			...
			$this->form_validation->set_rules('confirm', 'lang:password_confirm', 'matches[password]');
		}
